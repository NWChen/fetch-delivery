...
def search():
  ...
  cafes = []
  if category == 'cafe':
    cursor = g.conn.execute('SELECT C.name FROM Cafe C WHERE C.name LIKE (%s)', '%' + query + '%')
    for result in cursor:
      entry_dict = {"cname" : result[0]}
      cafes.append(entry_dict)
    cursor.close()
  elif category == 'neighborhood':
    cursor = g.conn.execute('SELECT C.name FROM Cafe C, Located_At La, Location L WHERE C.cid = La.cid AND La.lid = L.lid AND L.neighborhood LIKE (%s)', '%' + query + '%')
    for result in cursor:
      entry_dict = {"cname" : result[0]}
      cafes.append(entry_dict)
    cursor.close()
...
def create_account():
  ...
  if request.method == 'POST':
    ...
    if password == "" or first_name == "" or last_name == "" or username == "":
...
def add():
  ...
  if request.method == 'POST':
    ...
    try:
      opening_time = datetime.datetime.strptime(hours.split("-")[0].strip(), '%I:%M%p')
      closing_time = datetime.datetime.strptime(hours.split("-")[1].strip(), '%I:%M%p')
    except:
      error = "Incorrect format for cafe hours."
      return render_template('add_cafe.html', error=error)

    valid_cafe = (cafe_name != "")
    ...
    if valid_cafe:
      ...
      try:
        ...
        error = "Error with cafe information format."
        return render_template('add_cafe.html', error=error)

      if building_number or street_name or neighborhood:
        ...
        try:
          ...
          error = "Error with cafe location format."
          return render_template('add_cafe.html', error=error)
      return redirect('/')
    else:
      error = "This is not a valid cafe name! Cafe names cannot be blank or repeats."
  return render_template('add_cafe.html', error=error)
